...
from fastapi_app.services.qwen_service import QwenService
...
class ResumeService:
    ...
    async def extract_structured_data(text: str, skills: List[str]) -> dict:
        ...
        if not QwenService.is_configured() or not text.strip():
            return fallback
...
"""
    ...
    async def suggest_improvements(text: str) -> List[dict]:
        fallback = [
            {
                "issue": "OpenRouter Qwen is not configured",
                "recommendation": "Set OPENROUTER_API_KEY in backend/.env, then restart the FastAPI service.",
                "rationale": "Resume suggestions require an OpenRouter key for the configured Mistral model.",
                "priority": "medium",
            }
        ]
...
{text[:14000]}
"""
        return await QwenService.generate_json(prompt, fallback, "You are an expert resume writer.")
...
{text[:14000]}
"""
        return await QwenService.generate_json(prompt, [], "You are an expert recruiter and resume formatter.")

    @staticmethod
    async def analyze_quality(text: str) -> dict:
        fallback = {
            "overallScore": 0,
            "summary": "Set OPENROUTER_API_KEY to enable resume quality scoring through OpenRouter.",
        }
        if not QwenService.is_configured():
            return fallback
...
{text[:14000]}
"""
        return await QwenService.generate_json(prompt, fallback, "You are an expert career coach.")
